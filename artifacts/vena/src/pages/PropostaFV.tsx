...
import {
  PropostaLayoutEditor,
  LayoutConfig,
  DEFAULT_LAYOUT,
} from "./PropostaLayoutEditor";
...
export function PropostaFV() {
  ...
  const [layoutSalvando, setLayoutSalvando] =
    useState(false);

  const [aba, setAba] = useState<
    "form" | "layout" | "preview" | "arquivos"
  >("form");
    ...
    apiCall("/proposta-layout")
      .then((data) => {
        ...
        setLayout({
          ...
          identidade: {
            ...DEFAULT_LAYOUT.identidade,
            ...data.identidade,
          },

          cores: {
            ...DEFAULT_LAYOUT.cores,
            ...data.cores,
          },

          fontes: {
            ...DEFAULT_LAYOUT.fontes,
            ...data.fontes,
          },

          opcoes: {
            ...DEFAULT_LAYOUT.opcoes,
            ...data.opcoes,
          },
  ...
  const gerar = useCallback(() => {
    ...
    const linhasFiltradas = pagamento.filter((l) => {
      ...
      if (!formas.financiamento && l.forma.toLowerCase().includes("financ"))
        return false;
      if (!formas.parcelado && l.forma.toLowerCase().includes("parcel"))
        return false;
      return true;
    });
    ...
    return html;
  }, [form, pagamento, formas, layout]);

  useEffect(() => {
    if (aba !== "preview") return;
  ...
  async function salvarLayout() {
    setLayoutSalvando(true);

    try {
      await apiCall("/proposta-layout", {
        method: "PUT",
    ...
    try {
      const res = await fetch("/api/automation/generate-pdf", {
        ...
        body: JSON.stringify({
          ...
          clientId: null,
          proposalId: null,
          fileName: `proposta-${form.nomeCliente}-${new Date()
            .toISOString()
            .split("T")[0]}.pdf`,
        }),
      });
      ...
      a.href = url;
      a.download = `proposta-${form.nomeCliente}-${new Date()
        .toISOString()
        .split("T")[0]}.pdf`;
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">
            Proposta Fotovoltaica
          </h2>
          <p className="text-muted-foreground">
            Preencha os dados e gere a proposta personalizada.
          </p>
        </div>
      ...
      <div className="flex gap-1 border-b">
        {(["form", "layout", "preview", "arquivos"] as const).map((t) => (
          <button
            key={t}
            onClick={() => {
              if (t === "preview") gerar();
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className="text-xs font-mono text-orange-600 border-orange-300"
                >
                  mala direta
                </Badge>
              </CardTitle>
...
<Card>
  <CardHeader className="pb-3">
    <CardTitle className="text-sm flex items-center gap-2">
      ...
      <Badge variant="outline" className="text-xs font-mono text-green-700 border-green-400">
        tabela
      </Badge>
    </CardTitle>
  </CardHeader>
  ...
  <CardContent className="space-y-4">
    ...
    <div className="flex flex-wrap gap-3">
      {([
        ...
        <label
          key={key}
          className={`flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border text-sm font-medium transition-all ${
            formas[key]
    ...
    {pagamento.length > 0 && (
      <div className="rounded-lg overflow-hidden border">
        ...
        {pagamento.map((linha, idx) => (
          <div
            key={idx}
            className="grid grid-cols-[1fr_120px_160px_40px] gap-0 px-3 py-2 border-t items-center"
          >
            <Input
              ...
              value={linha.forma}
              onChange={(e) =>
                updateLinha(idx, "forma", e.target.value)
              }
            />
            ...
            <Input
              ...
              value={linha.qtd}
              onChange={(e) =>
                updateLinha(idx, "qtd", e.target.value)
              }
            />
            ...
            <Input
              ...
              value={linha.valor}
              onChange={(e) =>
                updateLinha(idx, "valor", e.target.value)
              }
            />
